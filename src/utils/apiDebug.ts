...
export const initializeApiDebug = (user?: any) => {
  ...
  const allowed = process.env.NODE_ENV !== 'production' || canUseApiDebug(user);
...
export const setApiDebugEnabled = (enabled: boolean, user?: any) => {
  if ((process.env.NODE_ENV === 'production' && !canUseApiDebug(user)) || typeof window === 'undefined') return false;
