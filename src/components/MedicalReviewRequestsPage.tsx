...
const MedicalReviewRequestsPage: React.FC = () => {
  ...
  const [translatingClientVisibleAdminNote, setTranslatingClientVisibleAdminNote] = useState(false);
  ...
  const handleTranslateClientVisibleAdminNote = async () => {
    ...
    const language = profileLanguage.startsWith('pl') || profileLanguage === 'polish' || profileLanguage === 'polski'
      ? 'pl'
      : profileLanguage.startsWith('cs') || profileLanguage.startsWith('cz') || profileLanguage === 'czech' || profileLanguage === 'cesky' || profileLanguage === 'česky' || profileLanguage === 'cestina' || profileLanguage === 'čeština'
        ? 'cs'
        : 'en';
    if (language === 'en') { setClientVisibleAdminNoteStatus('The client profile language is English.'); return; }
    try {
      setTranslatingClientVisibleAdminNote(true);
      await medicalReviewRequestsApi.updateClientVisibleAdminNote(selected._id, clientVisibleAdminNoteSource);
      const response = await medicalReviewRequestsApi.translateClientVisibleAdminNote(selected._id, language);
      const translated = (response.data as any)?.clientVisibleAdminNoteTranslations?.[language]?.text
        || (response.data as any)?.data?.clientVisibleAdminNoteTranslations?.[language]?.text;
      if (!translated) throw new Error('Translation returned no text.');
      setSelected(response.data);
      setClientVisibleAdminNote(translated);
      setClientVisibleAdminNoteStatus(`Translated to ${language === 'pl' ? 'Polish' : 'Czech'} for this client.`);
    } catch (error: any) {
      setClientVisibleAdminNoteStatus(error?.response?.data?.message || error?.message || 'Unable to translate the client message.');
    } finally {
      setTranslatingClientVisibleAdminNote(false);
    }
  };
  ...
  const renderMedicalSummary = () => {
    ...
    return <section id="mrr-medical-summary" className="rounded-xl border border-slate-300 bg-white p-4 shadow-sm sm:p-5">
      ...
      <div className="mt-4 rounded-lg border border-blue-200 bg-blue-50 p-4">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="text-sm font-semibold text-blue-950">Reviewer-language equivalent</div><div className="mt-1 text-xs text-blue-800">Generated from the records below in the advisor's configured review language. The original records remain authoritative.</div></div><button type="button" disabled={generatingMedicalSummary} onClick={handleGenerateMedicalSummary} className="no-print rounded-md bg-blue-700 px-3 py-2 text-xs font-semibold text-white hover:bg-blue-800 disabled:opacity-60">{generatingMedicalSummary ? 'Generating…' : 'Generate reviewer equivalent'}</button></div>
        {generatedMedicalSummary ? <div className="mt-3"><div className="mb-2 inline-flex rounded-full bg-blue-100 px-2 py-1 text-[11px] font-bold uppercase text-blue-900">{generatedMedicalSummary.language || 'English'} equivalent</div><div className="whitespace-pre-wrap rounded-md bg-white/80 p-3 text-sm text-blue-950">{generatedMedicalSummary.summary}</div><div className="mt-2 text-[11px] text-blue-800">Generated {formatDateTime(generatedMedicalSummary.generatedAt)} · {generatedMedicalSummary.generatedBy === 'openai' ? `AI (${generatedMedicalSummary.model || 'OpenAI'})` : 'Source rules fallback'}{generatedMedicalSummary.unavailableReason ? ` · ${generatedMedicalSummary.unavailableReason}` : ''} · Verify against original source files.</div></div> : <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-blue-950">{generatedOverview.map((line) => <li key={line}>{line}</li>)}</ul>}
      </div>
                ...
                >
                  ...
                  {isReadOnlyView ? (
                    ...
                    <>
                      ...
                      />
                      <div className="mrr-decision-buttons mt-2">
                        {decisionOptions.map((option) => (
                          <button
                            key={option}
                            type="button"
