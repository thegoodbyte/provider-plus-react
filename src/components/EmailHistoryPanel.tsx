...
import { FiChevronDown, FiChevronUp, FiInbox, FiMail, FiPlus, FiRefreshCw, FiX } from 'react-icons/fi';
...
const EmailHistoryPanel: React.FC<EmailHistoryPanelProps> = ({ clientId, bookingId, retreatId, title = 'Email history', subtitle, recipientEmail, recipientName }) => {
  ...
  const [selectedEmail, setSelectedEmail] = useState<{ kind: 'sent'; email: SentEmail } | { kind: 'received'; email: InboundEmail } | null>(null);
  ...
  const openSentEmail = async (email: SentEmail) => {
    setSelectedEmail({ kind: 'sent', email });
    if (!email._id) return;
    try {
      setDetailLoading(true);
      const response = await communicationsApi.getSentEmail(email._id);
      setSelectedEmail({ kind: 'sent', email: response.data });
    } catch {
      // The list record normally contains the full message; keep it visible if detail loading fails.
    } finally {
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4 sm:p-5">
              ...
              combinedRows.map((row) => {
                if (row.kind === 'sent') {
                  ...
                  return (
                    <tr
                      key={row.id}
                      onClick={() => void openSentEmail(email)}
                ...
                return (
                  <tr
                    key={row.id}
                    onClick={() => setSelectedEmail({ kind: 'received', email })}
                    className="cursor-pointer hover:bg-blue-50"
                    title="Open email"
                  >
                    <td className="px-4 py-3">
      ...
      {selectedEmail && (
        <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/40 p-3 sm:p-6" onClick={() => setSelectedEmail(null)}>
          <div className="flex max-h-[calc(100vh-24px)] w-full max-w-3xl flex-col overflow-hidden rounded-xl bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="flex shrink-0 items-start justify-between gap-4 border-b border-gray-200 px-4 py-4 sm:px-6">
              <div className="min-w-0">
                <div className="text-xs font-semibold uppercase tracking-wide text-gray-500">{selectedEmail.kind === 'sent' ? 'Sent email' : 'Received email'}</div>
                <h3 className="mt-1 break-words text-lg font-semibold text-gray-900">{selectedEmail.email.subject || '(no subject)'}</h3>
              </div>
              <button type="button" onClick={() => setSelectedEmail(null)} className="rounded-md p-2 text-gray-500 hover:bg-gray-100" aria-label="Close email">
                <Icon icon={FiX} className="h-5 w-5" />
              </button>
            </div>
            <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-6">
              {detailLoading && <div className="mb-3 text-sm text-gray-500">Loading full message…</div>}
              {selectedEmail.kind === 'sent' ? (
                <div className="space-y-2 text-sm">
                  <div><span className="font-medium text-gray-500">To:</span> {(selectedEmail.email.to || []).join(', ') || '-'}</div>
                  <div><span className="font-medium text-gray-500">CC:</span> {(selectedEmail.email.cc || []).join(', ') || 'None'}</div>
                  <div><span className="font-medium text-gray-500">Sent:</span> {formatDate(selectedEmail.email.sentAt || selectedEmail.email.createdAt)}</div>
                  <div><span className="font-medium text-gray-500">Status:</span> {selectedEmail.email.status || 'queued'}</div>
                  <div className="mt-4 rounded-lg border border-gray-200 bg-gray-50 p-4">
                    <pre className="whitespace-pre-wrap break-words font-sans text-sm leading-6 text-gray-800">{selectedEmail.email.bodyText || 'No message body stored.'}</pre>
                  </div>
                </div>
              ) : (
                <div className="space-y-2 text-sm">
                  <div><span className="font-medium text-gray-500">From:</span> {selectedEmail.email.fromName ? `${selectedEmail.email.fromName} <${selectedEmail.email.fromEmail || ''}>` : selectedEmail.email.fromEmail || '-'}</div>
                  <div><span className="font-medium text-gray-500">To:</span> {(selectedEmail.email.to || []).join(', ') || '-'}</div>
                  <div><span className="font-medium text-gray-500">Received:</span> {formatDate(selectedEmail.email.receivedAt || selectedEmail.email.createdAt)}</div>
                  <div className="mt-4 rounded-lg border border-gray-200 bg-gray-50 p-4">
                    <pre className="whitespace-pre-wrap break-words font-sans text-sm leading-6 text-gray-800">{selectedEmail.email.bodyText || selectedEmail.email.snippet || 'No message body stored.'}</pre>
                  </div>
                </div>
              )}
            </div>
