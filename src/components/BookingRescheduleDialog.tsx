...
type Props = {
  currentRetreatId: string; retreats: Retreat[]; saving: boolean; error?: string; onClose: () => void;
  onSubmit: (data: { targetRetreatId: string; reason: string; note: string; sendEmail: boolean }) => Promise<void>;
};

export default function BookingRescheduleDialog({ currentRetreatId, retreats, saving, error, onClose, onSubmit }: Props) {
  ...
  const options = useMemo(() => retreats
    ...
    .sort((left: any, right: any) => new Date(left.startDate).getTime() - new Date(right.startDate).getTime()), [currentRetreatId, retreats]);
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="dialog" aria-modal="true" aria-labelledby="reschedule-title">
    <form className="w-full max-w-2xl rounded-xl bg-white p-6 shadow-2xl" onSubmit={(event) => { event.preventDefault(); void onSubmit({ targetRetreatId, reason, note, sendEmail }); }}>
      ...
      <label className="mt-5 block text-sm font-semibold">Move to retreat
        <select required value={targetRetreatId} onChange={(event) => setTargetRetreatId(event.target.value)} className="mt-2 w-full rounded border border-gray-300 p-3">
          <option value="">Select a future retreat</option>
          {options.map((retreat: any) => {
            const places = availablePlaces(retreat); const full = places === 0;
            return <option key={id(retreat)} value={id(retreat)} disabled={full}>{code(retreat)} · {date(retreat.startDate)} – {date(retreat.endDate)} · {full ? 'Full' : places === null ? 'Capacity not set' : `${places} places open`}</option>;
          })}
        </select>
      </label>
