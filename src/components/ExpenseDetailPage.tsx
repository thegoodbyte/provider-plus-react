...
const ExpenseDetailPage: React.FC = () => {
  ...
  const paymentMethod = typeof expense.paymentMethodId === 'object' ? expense.paymentMethodId.name : expense.paymentMethodId || 'Not specified';
  const rows = [['Date', new Date(expense.expenseDate).toLocaleDateString()], ['Item', expense.description || '—'], ['Vendor', expense.vendor || '—'], ['Category', type], ['Payment method', paymentMethod], ['Retreat', retreat], ['Status', expense.status], ['Created', expense.createdAt ? new Date(expense.createdAt).toLocaleString() : '—']];
  return (
