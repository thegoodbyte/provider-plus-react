...
describe('BookingOverviewPanel', () => {
  beforeEach(() => { jest.clearAllMocks(); (paymentsApi.getByBooking as jest.Mock).mockResolvedValue({ data: [{ amount: 30, status: 'completed' }] }); (paymentRequestsApi.getByBooking as jest.Mock).mockResolvedValue({ data: [] }); (useBookingRequirements as jest.Mock).mockReturnValue({ rows: [requirement], items: [], loading: false }); });
  ...
  it('does not treat a pending confirmation step as sent', () => { expect(sentConfirmationStep([{ key: 'booking_confirmation_sent', status: 'pending' }])).toBeUndefined(); });
});
