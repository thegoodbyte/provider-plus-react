...
describe('BookingOverviewPanel', () => {
  beforeEach(() => { jest.clearAllMocks(); (paymentsApi.getByBooking as jest.Mock).mockResolvedValue({ data: [{ amount: 30, currency: 'USD', status: 'completed' }] }); (paymentRequestsApi.getByBooking as jest.Mock).mockResolvedValue({ data: [] }); (useBookingRequirements as jest.Mock).mockReturnValue({ rows: [requirement], items: [], loading: false }); });
