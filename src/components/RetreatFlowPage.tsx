...
const RetreatFlowPage: React.FC = () => {
        ...
        >
          ...
          {emailTemplates.map((template) => (
            <option key={template._id} value={template._id}>
              {template.name} ({template.templateKey || template.category || 'general'} / {template.language || 'en'})
            </option>
          ))}
        </select>
        <p className="mt-2 text-xs text-gray-500">
          Choose the default template for this step. Sending uses the client's language from an active template with the same variant key, then English, then this template.
        </p>
      </div>
  ...
  if (loading && retreats.length === 0 && templates.length === 0) {
    return <LoadingSpinner message="Loading retreat readiness setup..." />;
  }
  ...
  if (!selectedRetreatId) {
    return (
      <div className="p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-semibold text-gray-900">Retreat Readiness Setup</h1>
          <p className="text-sm text-gray-600">Choose a retreat to edit its readiness step setup.</p>
        </div>
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Retreat Readiness Setup</h1>
          <p className="text-sm text-gray-600">{selectedRetreat?.name || 'Selected retreat'} • configure retreat-specific steps, deadlines, emails, and colors.</p>
        </div>
