...
const MedicationStopPlanPage: React.FC = () => {
  ...
  const save = async () => {
    ...
    try {
      await bookingFlowApi.saveMedicationStopPlan(bookingId, allClear ? [] : entries.filter(entry => entry.name || entry.instruction), allClear, allClearNote);
