...
const empty = { name: '', email: '', phone: '', notes: '', isActive: true };
    ...
    <div className="mx-auto max-w-5xl p-4">
      ...
      <div className="space-y-3">
        {rows.map((row) => <div key={row._id} className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <div><div className="text-lg font-bold text-slate-950">{row.name} {row.isActive === false && <span className="text-sm font-semibold text-slate-400">Inactive</span>}</div><div className="text-sm text-slate-500">{[row.email, row.phone].filter(Boolean).join(' · ') || 'No contact details'}</div>{row.notes && <div className="mt-1 text-sm text-slate-700">{row.notes}</div>}</div>
          <div className="flex gap-2">
            <button onClick={() => { setEditingId(row._id || ''); setForm({ name: row.name, email: row.email || '', phone: row.phone || '', notes: row.notes || '', isActive: row.isActive !== false }); }} className="flex min-h-11 items-center gap-2 rounded-xl bg-slate-100 px-4 font-bold"><Pencil size={17} />Edit</button>
