...
const SubmissionNotificationsPage: React.FC<NotificationScope> = ({ clientId, bookingId, title = 'Notifications', subtitle }) => {
  ...
  const load = useCallback(async () => { setLoading(true); setError(''); try { const response = await api.get('/submission-notifications', { params: { search: search || undefined, status: status || undefined, unreadOnly: unreadOnly || undefined, view, clientId: clientId || undefined, bookingId: bookingId || undefined } }); setRows(response.data || []); } catch (e: any) { setError(e.response?.data?.message || 'Unable to load notifications.'); } finally { setLoading(false); } }, [search, status, unreadOnly, view, clientId, bookingId]);
