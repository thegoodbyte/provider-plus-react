...
const SubmissionNotificationsPage: React.FC<NotificationScope> = ({ clientId, bookingId, title = 'Notifications', subtitle }) => {
  ...
  const [purging, setPurging] = useState(false); const [purgeMessage, setPurgeMessage] = useState('');
  ...
  const purgePastRetreats = async () => {
    setPurging(true); setError(''); setPurgeMessage('');
    try {
      const response = await api.patch('/submission-notifications/purge-past-retreats');
      const markedCount = response.data?.markedCount || 0;
      setPurgeMessage(markedCount ? `${markedCount} past-retreat notification${markedCount === 1 ? '' : 's'} marked read.` : 'No unread past-retreat notifications to purge.');
      await load();
      window.dispatchEvent(new Event('notifications-updated'));
    } catch (e: any) { setError(e.response?.data?.message || 'Unable to purge past-retreat notifications.'); }
